...
- **Warehouse credentials (data vault sources).** Sources are stored in `~/.heap-mcp/sources.json` (created `0600`, directory `0700`). Prefer `${ENV_VAR}` references over literal secrets in that file; tools mask secret fields in output and startup logs print source names only. Grant the warehouse user read-only access to the Heap schema — query tools enforce SELECT-only guardrails, and least privilege backs that up.
- **Source admin tools.** `heap_add_source`/`heap_remove_source` write to disk and open outbound connections, so they are disabled by default under the unauthenticated `TRANSPORT=http` (a network caller must not get them). `HEAP_ALLOW_SOURCE_ADMIN=true` overrides this — only do that behind your own auth layer.
